...
class LoadImages:
    ...
    def __next__(self):
        ...
        if self.count == self.nf:
            # raise StopIteration
            timer = 0
            while(self.count == self.nf):
                if timer%50_000 == 0:
                    print("Waiting for frame...")
                
                if '*' in self.path:
                    files = sorted(glob.glob(self.path, recursive=True))  # glob
                elif os.path.isdir(self.path):
                    files = sorted(glob.glob(os.path.join(self.path, '*.*')))  # dir
        ...
        path = self.files[self.count]
        # print(f"path name: {path}, self.count: {self.count}, self.nf{self.nf}")
